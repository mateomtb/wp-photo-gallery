...
class autoblogcron {
	...
	function test_feed($feed_id, $ablog) {
		...
		if ( !function_exists('fetch_autoblog_feed') && file_exists(ABSPATH . WPINC . '/feed.php') ) {
			require_once (ABSPATH . WPINC . '/feed.php');
		}

		// Check again in case the load didn't work
		if ( !function_exists('fetch_autoblog_feed') ) {
		...
		$feed = fetch_autoblog_feed($ablog['url']);
	...
	public function process_feed( $feed_id, $ablog ) {
		...
		for ( $x = 0; $x < $max; $x++ ) {
			...
			$post_title = trim( $item->get_title() );
			$post_content = trim( html_entity_decode( $item->get_description(), ENT_QUOTES, 'UTF-8' ) );
