<?php
/*                                                                                                                                                                                                                                                             
Plugin Name: DFM Syndication
Plugin URI: http://173.45.226.6/wp-content/plugins/dfm-broadcast
...
require_once('DFM_Broadcast_3Base.php');
class DFM_Broadcast extends DFM_Broadcast_3Base
{
	...
	{
		add_submenu_page('settings.php', 'DFM Syndication', 'Syndication', 'activate_plugins', 'DFM_Broadcast', array (&$this, 'admin'));
	}

	public function add_menu()
	{
		add_options_page('DFM Broadcast', __('Syndication'), 'publish_posts', 'DFM_Broadcast', array (&$this, 'user'));
	...
	public function admin_print_styles()
	{
		...
		$pages = array(get_class(), 'DFM_Activity_Monitor');
		...
		wp_enqueue_script( 'dfm_broadcast', '/' . $this->paths['path_from_base_directory'] . '/js/user.js' );
		wp_enqueue_style('dfm_broadcast', '/' . $this->paths['path_from_base_directory'] . '/css/DFM_Broadcast.css', false, '0.0.1', 'screen' );
	}
	...
	public function activate()
	{
		...
		$this->query("CREATE TABLE IF NOT EXISTS `".$this->wpdb->base_prefix."_dfm_broadcast` (
		...
		$this->query("CREATE TABLE IF NOT EXISTS `".$this->wpdb->base_prefix."_dfm_broadcast_broadcastdata` (
	...
	{
		parent::uninstall();
		$this->query("DROP TABLE `".$this->wpdb->base_prefix."_dfm_broadcast`");
		$this->query("DROP TABLE `".$this->wpdb->base_prefix."_dfm_broadcast_broadcastdata`");
	}
	...
	public function user()
	{
		...
		{
			$tab_data['tabs'][] = __('DFM Syndication groups');
	...
	protected function admin_settings()
	{
		...
		$inputs = array(
			'role_broadcast' => array(
				'name' => 'role_broadcast',
				'type' => 'select',
				'label' => 'Syndication access role',
				'value' => $this->get_site_option('role_broadcast'),
				'description' => 'The syndication access role is the user role required to use the syndicate function at all.',
				'options' => $roles,
			...
			'role_link' => array(
				...
				'description' => 'When a post is linked with syndicated posts, the child posts are updated / deleted when the parent is updated.',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'role_broadcast_as_draft' => array(
				...
				'description' => 'Which role is needed to syndicate drafts?',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'role_broadcast_scheduled_posts' => array(
				...
				'description' => 'Which role is needed to syndicate scheduled (future) posts?',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'role_groups' => array(
			...
			'role_categories' => array(
				...
				'description' => 'Which role is needed to allow category syndicating? The categories must have the same slug.',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			...
			'role_tags' => array(
				...
				'description' => 'Which role is needed to allow tag syndicating? The tags must have the same slug.',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			...
			'role_custom_fields' => array(
				...
				'description' => 'Which role is needed to allow custom field syndicating?',
				'options' => $roles,
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'save_post_priority' => array(
				...
				'size' => 3,
				'maxlength' => 10,
				'description' => 'A higher save-post-action priority gives other plugins more time to add their own custom fields before the post is syndicated. <em>Raise</em> this value if you notice that plugins that use custom fields aren\'t getting their data syndicated, but 640 should be enough for everybody.',
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'custom_field_exceptions' => array(
				...
				'cols' => 30,
				'rows' => 5,
				'description' => 'Custom fields that begin with underscores (internal fields) are ignored. If you know of an internal field that should be syndicated, write it down here. One custom field key per line and it can be any part of the key string.',
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'override_child_permalinks' => array(
				...
				'description' => 'This will force child posts (those syndicated to other sites) to keep the original post\'s permalink. If checked, child posts will link back to the original post on the original site.',
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
			'save' => array(
				'name' => 'save',
	...
	protected function admin_post_types()
	{
		...
		echo '<p>Choose which post types should be allowed to use syndication.</p>
	...
	protected function admin_required_list()
	{
		...
		echo '
			...
			<p>
				The required list is used to force users to syndicate to the National site. The National site ignores this list.
			</p>
			...
			<p>Select which blogs the user will be required to syndicate to.</p>
	...
	protected function admin_blacklist()
	{
		...
		echo '
			...
			<p>The blacklist specifies which blogs the users may never syndicate to, even if they\'ve got write access. National site ignores this list. All sites other than National MUST be checked to avoid duplicate content and to maintain the workflow integrity.</p>
	...
	protected function admin_activity_monitor()
	{
		...
		$inputs = array(
			'activity_monitor_broadcasts' => array(
				...
				'description' => 'Reports when a user has used the syndicate function.',
				'make_table_row' => true,		// Just a marker to tell the function to automake this input in the table.
			),
		...
		echo '
			...
			<p>
				If you want DFM Syndicate to send notifications of user activity to DFM Activity Monitor,
			...
			foreach($data['groups'] as $groupID=>$ignore)
			{
				if (isset($_POST['broadcast']['groups'][$groupID]))
				{
					...
					if ( $this->get_option('activity_monitor_group_changes') )
					{
						...
						$blog_text .= '.';
						do_action('DFM_activity_monitor_new_activity', array(
	...
	protected function user_unlink()
	{
		...
		if (!wp_verify_nonce($nonce, $nonce_key))
			die("Security check: not supposed to be unlinking syndicated post!");
	...
	*/
	protected function user_trash()
	{
		...
		$message = __('The syndicated child post has been put in the trash.');
	...
	private function add_meta_box($type)
	{
		global $blog_id;
		global $post;
		global $linkedParent; // Linked Parent Info
		...
		$linkedParent = $broadcast_data->get_linked_parent();

		if($blog_id != 1) { // National can ignore linked content. All sites must link content to National to get the ID of the originating post so we do not syndicate back to them
			if ( $broadcast_data->get_linked_parent() !== false)
			{
				echo '<p>';
				//print_r($linked_parent); //debug
				//echo 'Parent ID: '.$linkedParent['blog_id']; //debug
				echo __('<br />This post is a syndicated child post. It cannot be syndicated further.');
				echo '</p>';
				return;
			}
		}
		...
		if ($this->role_at_least( $this->get_site_option('role_link') ))
		{
			...
			$inputBroadcastLink = array(
				'name' => 'link',
				'type' => 'hidden', // Hide fields from users and make them required
				...
				echo $form->makeInput($inputBroadcastLink);
		}
		...
		if ($this->role_at_least( $this->get_site_option('role_categories_create') ) && $post_type_supports_categories)
		{
			...
			echo $form->makeInput($inputCategoriesCreate);
		}
		...
		if ($this->role_at_least( $this->get_site_option('role_tags_create') ) && $post_type_supports_tags)
		{
			...
			echo $form->makeInput($inputTagsCreate);
		}
		...
		if ($this->role_at_least( $this->get_site_option('role_custom_fields') ) && $post_type_supports_custom_fields)
		{
			$inputCustomFields = array(
				...
				'value' => 'custom_fields',
				'checked' => true,
				'title' => __('Syndicate all the custom fields and the featured image?'),
				'label' => __('Syndicate custom fields'),
			);
			echo $form->makeInput($inputCustomFields);
		}
		...
		//Remove originating blog for National list
		if ($blog_id == 1) {
			$parentID = $linkedParent['blog_id'];
			unset($blogs[$parentID]);		
		} else {
			$postStatus = $post->post_status;
			if ( $postStatus != 'draft' && $postStatus != 'pending') {
				$nationalID = 1;
				unset($blogs[$nationalID]); // Keep syndicated content from being re-broadcast once published if it is not from National
			}
		}
		...
		if ( count($blogs) > 0 ) {	
			echo '<div class="broadcast_to">
					<p class="howto">'. __('Syndicate to:') .'</p>
					<div class="blogs">
			...
			echo '<p>';
			echo __('<br />This post has already been sent to the National site. It cannot be syndicated again.');
			echo '</p>';
		}
	}
	...
	public function save_post($post_id)
	{
		...
		// Add required blogs.
		global $current_blog;
        $currentID = $current_blog->blog_id; // Exclude National site from using the required list
        if ($currentID != 1) {
			if ($this->get_site_option('always_use_required_list'))
			{
				$requiredBlogs = $this->get_required_blogs();
		...
		if ($custom_fields)
		{
			// Add permalink as custom field when publishing to National for Canonical URL
			if ($blog_id != 1) { 
				$postStatus = $newPost['post_status'];
				if ( $postStatus != 'draft' && $postStatus != 'pending') {
					$permaLink = get_permalink($post_id);
					add_post_meta($post_id, 'syndication_source_uri', $permaLink, true);
				}
			}
		...
		foreach($blogs as $blogID)
		{
			...
			$postStatus = $newPost['post_status'];
			//$hasCat = has_category('Syndicated',$newPost->ID);
			global $current_blog;
			$currentID = $current_blog->blog_id;

			if ($currentID != 1) {			
				if ($postStatus == 'draft' || $postStatus == 'pending' ) { // Don't syndicate content to National that is a draft
					$need_to_insert_post = false;
				}
				// Check if the post already exists
				elseif ( $newPost["post_modified_gmt"] != $newPost["post_date_gmt"] )
				{
					$need_to_insert_post = false;
				}
			}
		
			if ($currentID == 1) { // Exclude child blogs from linking parent to National
				if ($link) {
			...
			{	
				if ($blog_id == 1) { // Set all broadcast posts to Draft status if they do not come from National site
			...
			if ($categories)
			{
				...
				foreach($post_categories as $post_category)
				{
					...
					if ($blog_id != 1 ) { // Do not create new categories on National Site - Jason Armour 2.9.12
				...
				if ( count($categories_to_add_to) > 0 )
					if ($blog_id == 1) { // Reset broadcasted content to Syndicated category only in National Site automatically - Jason Armour 2.9.12
						$categories_to_add = array();
					  	$categories_to_add[$target_blog_category_id] = 3; 
						wp_set_post_categories( $new_post_id, $categories_to_add );
					} else {
			...
			*/
			...
			if ($custom_fields)
			{
				...
				// Attached files are custom fields... but special custom fields. Therefore they need special treatment.
				if ($has_thumbnail)
				{
		...
		if ( $this->get_option('activity_monitor_broadcasts') )
		{
			...
			do_action('DFM_activity_monitor_new_activity', array(
				'activity_type' => 'dfm_broadcasted',
				'tr_class' => 'activity_monitor_broadcast activity_monitor_broadcasted',
				'activity' => array(
					'' => '%user_display_name_with_link% has syndicated '.$post_url_and_name.' to: ' . implode(', ', $to_broadcasted_blogs),
				),
			));
		}
	...
	{
		global $current_blog;
		$currentID = $current_blog->blog_id; // Show which sites content was syndicated to on National site - Jason Armour 2.14.12
		if ($currentID == 1)
			$defaults['dfm_broadcast'] = '<span title="'.__('Shows which blogs have posts linked to this one').'">'.__('Syndicated To').'</span>';
			return $defaults;
	}	
	
	public function manage_posts_custom_column($column_name, $post_id)
	{
		if ($column_name != 'dfm_broadcast')
			return;
		...
		if ( $broadcast_data->has_linked_children() )
		{
			...
			foreach($children as $blogID => $postID)
			{
				...
				$urlUnlink = wp_nonce_url("options-general.php?page=DFM_Broadcast&amp;action=unlink&amp;post=$post_id&amp;child=$blogID", 'broadcast_unlink_' . $blogID . '_' . $post_id);
				$urlTrash = wp_nonce_url("options-general.php?page=DFM_Broadcast&amp;action=trash&amp;post=$post_id&amp;child=$blogID", 'broadcast_trash_' . $blogID . '_' . $post_id);
				$display[] = '<div class="broadcasted_blog"><a class="broadcasted_child" href="'.$urlChild.'">'.$blogs[$blogID]['blogname'].'</a>
					<div class="row-actions broadcasted_blog_actions">
						<small>
						<a href="'.$urlUnlink.'" title="'.__('Remove links to this syndicated child').'">'.__('Unlink').'</a>
						| <span class="trash"><a href="'.$urlTrash.'" title="'.__('Put this syndicated child in the trash').'">'.__('Trash').'</a></span>
						</small>
					</div>
				</div>
				';
			}
			echo '<ul style="display:none;"><li>' . implode('</li><li>', $display) . '</li></ul>';
		}
	}
	...
	public function post_row_actions($actions)
	{
		...
		if ($broadcast_data->has_linked_children())
			$actions = array_merge($actions, array(
				'broadcast_unlink' => '<a href="'.wp_nonce_url("options-general.php?page=DFM_Broadcast&amp;action=unlink&amp;post=$post->ID", 'broadcast_unlink_' . $post->ID).'" title="'.__('Remove links to all the syndicated children').'">'.__('Unlink').'</a>',
			));
		return $actions;
	}
	...
	public function create_meta_box()
	{
		if ($this->role_at_least( $this->get_site_option('role_broadcast') ))
		{
			...
			if ( $this->role_at_least('super_admin') || count($this->list_user_writable_blog($this->user_id()))> 0 )	// User always has at least one to write to, if he's gotten THIS far.
			{
				$this->load_language();
				$post_types = $this->get_site_option('post_types');
				foreach($post_types as $post_type => $ignore)
					add_meta_box('DFM_broadcast', __('DFM Syndicate'), array(&$this, 'add_meta_box_type'), $post_type, 'side', 'low' );
	...
	/**
	 * Returns whether the site admin has required the blog.
	 */
	...
	{
		global $current_blog;
		$currentID = $current_blog->blog_id; // Exclude National (root) site from using Blacklist - Jason Armour 2.2.12
		$blacklist = $this->get_site_option('blacklist');
		if ($blacklist == '' || $currentID == 1)
			return false;
	...
	*/
	...
	{
		$returnValue = $this->query("SELECT * FROM `".$this->wpdb->base_prefix."_dfm_broadcast` WHERE user_id = '$user_id'");
	...
	private function sqlUserSet($user_id, $data)
	{
		...
		$this->query("DELETE FROM `".$this->wpdb->base_prefix."_dfm_broadcast` WHERE user_id = '$user_id'");
		$this->query("INSERT INTO `".$this->wpdb->base_prefix."_dfm_broadcast` (user_id, data) VALUES ('$user_id', '$data')");
	}
	
	private function sqlBroadcastDataGet($blog_id, $post_id)
	{
		$returnValue = $this->query("SELECT data FROM `".$this->wpdb->base_prefix."_dfm_broadcast_broadcastdata` WHERE blog_id = '$blog_id' AND post_id = '$post_id'");
		$returnValue = @unserialize( base64_decode($returnValue[0]['data']) );		// Unserialize the data column of the first row.
		if ($returnValue === false)
			$returnValue = array();
		return $returnValue;
	...
	{
		$this->query("DELETE FROM `".$this->wpdb->base_prefix."_dfm_broadcast_broadcastdata` WHERE blog_id = '$blog_id' AND post_id = '$post_id'");
	}
	...
	private function sqlBroadcastDataUpdate($blog_id, $post_id, $data)
	{
		...
		$data = base64_encode($data);
		$this->sqlBroadcastDataDelete($blog_id, $post_id);
		$this->query("INSERT INTO `".$this->wpdb->base_prefix."_dfm_broadcast_broadcastdata` (blog_id, post_id, data) VALUES ('$blog_id', '$post_id', '$data')");
	}
}

$DFM_broadcast = new DFM_Broadcast();
?>
