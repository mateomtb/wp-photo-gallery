jQuery(document).ready(function($) {
	var blog_count = $("#DFM_broadcast .blogs ul.broadcast_blogs li").length;
	
	// Need to hide the blog list?
	if ( blog_count > 5 )
	{
		$("#DFM_broadcast .broadcast_to").addClass('broadcast_to_opened').append('<div class="arrow_container"><div class="arrow"></div></div>');
		
		close_broadcasted_blogs( $("#DFM_broadcast .broadcast_to") );
		
		// Make it switchable!
		$("#DFM_broadcast .broadcast_to .arrow_container").click(function(){
	...
	**/
	function close_broadcasted_blogs( item )
	{
		...
		$.each( $("#DFM_broadcast .blogs ul.broadcast_blogs li"), function (index,item){
