<?php
/*                                                                                                                                                                                                                                                             
Plugin Name: DFM Syndication Activity Monitor
Plugin URI: http://173.45.226.6/wp-content/plugins/dfm-activity-monitor
...
class DFM_Activity_Monitor extends SD_Activity_Monitor_Base
{
	...
	public function __construct()
	{
		...
		add_action( 'DFM_activity_monitor_cron', array(&$this, 'cron') );
		...
		add_action( 'DFM_activity_monitor_new_activity', array(&$this, 'log_new_activity'), 100000, 1 );
		add_filter( 'DFM_activity_monitor_delete_activity', array(&$this, 'delete_activity'), 100000, 2 );
		add_filter( 'DFM_activity_monitor_display_activity', array(&$this, 'display_activity'), 100000, 2 );
		add_filter( 'DFM_activity_monitor_find_activities', array(&$this, 'find_activities'), 10, 2 );
		add_filter( 'DFM_activity_monitor_list_activities', array(&$this, 'list_activities'), 10, 2 );
		add_filter( 'DFM_activity_monitor_list_activities', array(&$this, 'clean_activities_list'), 100000, 2 );		
		add_filter( 'DFM_activity_monitor_convert_activity_to_post', array(&$this, 'convert_activity_to_post'), 100000, 2 );
	}
	...
	public function activate()
	{
		...
		wp_schedule_event(time() + 600, 'daily', 'DFM_activity_monitor_cron');
		...
		{
			$activities = apply_filters( 'DFM_activity_monitor_list_activities', array() );
	...
	{
		parent::deactivate();
		wp_clear_scheduled_hook('DFM_activity_monitor_cron');
	}
	...
	{
		$this->common_admin_menu();
		add_submenu_page('settings.php', $this->_('Activity Monitor'), $this->_('Activity Monitor'), 'read', 'DFM_Activity_Monitor', array (&$this, 'admin'));
	}
	...
	public function common_admin_menu()
	{
		...
		if ($this->role_at_least( $this->get_site_option( 'role_logins_view_other') ))
		{
			...
			add_submenu_page('index.php', $this->_('Activity Monitor'), $this->_('Activity Monitor'), 'read', 'DFM_Activity_Monitor', array (&$this, 'admin'));
		}
	}
	...
	public function admin_print_styles()
	{
		...
		wp_enqueue_style('3wp_activity_monitor', '/' . $this->paths['path_from_base_directory'] . '/css/DFM_Activity_Monitor.css', false, '1.0', 'screen' );
	}
	...
	*/
	...
	public function publish_post($new_status, $old_status, $post)
	{
		...
		global $DFM_broadcast;
		if ( $DFM_broadcast !== null )
			if ( $DFM_broadcast->is_broadcasting() )
				return;
	...
	/**
		...
		After the whole $options is saved you can use the DFM_activity_monitor_display_activity to display your custom activity. 
	...
	**/
	public function list_activities($activities)
	{
		...
		{
			$activity['plugin'] = 'DFM Activity Monitor';
	...
	**/	
	...
	private function make_ip($type = 'text1')
	{
		...
		switch($type)		
		{
			...
			case 'html2':
				if ($_SERVER['REMOTE_HOST'] != '')
					return $_SERVER['REMOTE_HOST'] . ' <span class="DFM_activity_monitor_sep">|</span> '.$_SERVER['REMOTE_ADDR'];
				else
					return $_SERVER['REMOTE_ADDR'];
			break;
		}
	...
	private function show_activities($options)
	{
		...
		if ( $may_delete )
		{
			if ( isset($_POST['mass_submit']) )
			{
				...
				if ( count( $selected ) > 0 )
				{
					...
					{
						case 'delete':
							apply_filters( 'DFM_activity_monitor_delete_activity', $selected );
		...
		foreach( $options['activities'] as $activity )
		{
			...
			$activity = apply_filters( 'DFM_activity_monitor_display_activity', $activity );
			...
			if ( count( $activity['display_strings'] ) > 0 )
				foreach ($activity['display_strings'] as $data_key => $data_value)
					$activity_strings[] = sprintf('<span class="DFM_activity_monitor_activity_info_key">%s</span> <span class="activity_info_data">%s</span>', trim($data_key), $data_value);
		...
		return '
			...
			<table class="widefat DFM_activity_monitor">
				<thead>
					<tr>
						' . $th_cb . '
						<th>'.$this->_('Time').'</th>
...
$DFM_activity_monitor = new DFM_Activity_Monitor();
?>