 ...
 */
if ( !class_exists('PhpSmugException') ):
...
class phpSmug {
	...
	public function enableCache()
	{
        ...
        elseif ( $this->cacheType == 'apc' || $this->cacheType == 'apctest' ) 
        {
    ...
    private function getCached( $request )
	{
        ...
        {
            // echo '<!-- APC cache info: ' . apc_fetch($reqhash) . ' -->';
            return apc_fetch($reqhash);
        }
        elseif ( $this->cacheType == 'apctest' ) 
        {
            //print_r(apc_cache_info());
            echo apc_fetch('APC TEST');
            return apc_fetch('APC TEST');
        }
    	return FALSE;
    }
    ...
    private function cache( $request, $response )
	{
		...
		if ( ! strpos( $request['method'], '.auth.' ) ) {
            ...
            {
                echo $response;
                file_put_contents("apc_add", $reqhash . "\n===\n" . $response);
                return apc_add('APC TEST', 'APC Cache Test!');
            }
		}
        return TRUE;
    }
	...
	private function request( $command, $args = array() )
	{
		...
		// TODO: Cater for SmugMug being in read-only mode better.  At the moment we throw and exception and don't allow things to continue. JOSH K changed this - see below
		...
		if ( $this->parsed_response['stat'] == 'fail' ) {
			...
			//Instead of stopping the process and breaking the rotator, write logic for $this->error_code
			//For error information, you can display $this->error_msg
		} else {
	 ...
	 public function authorize()
	{
	  ...
	  * @return array
	  **/
	 protected static function processArgs( $arguments )
	 {
		$args = array();
...
if ( !class_exists('HttpRequestException') ):
    class HttpRequestException extends Exception {}
endif;

if ( !interface_exists('PhpSmugRequestProcessor') ):
...
if ( !class_exists('PhpSmugCurlRequestProcessor') ):
...
if ( !class_exists('PhpSmugSocketRequestProcessor') ):
