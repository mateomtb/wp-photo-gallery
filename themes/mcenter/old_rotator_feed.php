...
function outputWidgetData($the_path, $file_path){
	...
	$bcEmbedPattern = '/(https?:\/\/bcove.me\/[A-Za-z0-9]+)/'; // To check if post contains embedded brightcove
	//$bcEmbedPattern = '/bcove.me\//'; // To check if post contains embedded brightcove
	...
	if (count($returned_posts) == $number_of_items): //Making sure that the correct amount of data is in the array
		...
		foreach ($returned_posts as $post){
			...
			$excerpt = preg_replace($bcEmbedPattern, '', strip_tags($excerpt));
		...
		if ($_REQUEST['size'] != 'large'){
			foreach($returned_posts as $post){
				// Smugmug
				if ($image = get_post_meta($post->ID, 'smugdata', true)){
				...
				// BC embed image
				//elseif ($image = bcImageCheck($post, $bcEmbedPattern)) {
					//$image_paths[] = $image;
				//}
				// Backup
				else {
		...
		if ($_REQUEST['size'] == 'large'){
		   foreach($returned_posts as $k => $post){
				$width = $k == 0 ? 471 : 152;
				$height = $k == 0 ? 283 : 90;
				$rotator_thumb = $k == 0 ? 'rotator-large-thumb' : 'rotator-large-thumb-small';
				// Smugmug
				if ($image = get_post_meta($post->ID, 'smugdata', true)){
				...
				//elseif ($image = bcImageCheck($post, $bcEmbedPattern)) {
					//$image_paths_large[] = cacheCropImage($image, $width, $height, $the_path);
				//}
				// WP attachment image
				elseif ($image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), $rotator_thumb)) {
					// Works for BC embed images
					$image = $image[0];
